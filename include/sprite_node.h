 ...
 *
 * SpriteNode is a WorldNode that holds sprite. It's only supposed to be used
 * for rendering sprites that don't interact with the world, like background
 * tiles.
 */
class SpriteNode : public WorldNode {
 public:
     ...
     * we can get a reference to a texture tile with identifier Textures::Grass
     * like so
     * @code
    ...
    /**
     * Constructor that sets the sprite to use a texture from a reference and
     * sets a sprite rectangle. This is especially useful for tiles since we can
     * set it to repeat itself within the rectangle bounds by using the
     * setRepeated method from sf::Texture
     * @code
     * texture.setRepeated(true); //texture will repeat itself within the bounds
     * of the rectangle
     * @endcode
     */
    ...
    /**
     * Sets the node's identifier to a certain value to differentiate it from
     * others.
     */
    void setId(int id);
