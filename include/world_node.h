...
/**
 * @brief A world node is anything that exists and should be updated in the
 * world.
 *
 * A world node (or just node) is something that exists in the world, and is
 * updated and drawn (if it has something to be drawn) each frame. Since it
 * inherits from the sf::Transformable class, it has a position in the world and
 * has methods to change its scale, position or rotation. <br>
 * Nodes get attached to others in a tree structure. Each node has a pointer to
 * its parent node (@c nullptr if the node doesn't have a parent, i.e. it's a
 * root node), and can have several children attached to it using the
 * WorldNode::attachChild method.
 */
class WorldNode : public sf::Transformable,
                  public sf::Drawable,
 ...
 public:
     ...
     * attached to it, but is not a root node unless you call
     * WorldNode::setRoot().
     */
    WorldNode();
     ...
     *
     * The method will transfer ownership of the node to the calling node. After
     * the function is called, the passed node's parent will be the calling
     * node, and the calling node will have the passed node as a child.
     *
     ...
     *
     * After the function is called, the calling node will no longer have the
     * passed node as a child, and the passed node will have no parent. Children
     * of the passed node will not be affected by this.
     *
    ...
    /**
     * Returns the resulting transform after applying all the transforms
     * relative to its parents up until the root node, that is, until we reach a
     ...
     * which is the node at the top of the tree structure, ultimate parent of
     * all the nodes in play.
     */
    void setRoot();
     ...
     *
     * @return True if the node is going to be removed before the next frame,
     * false otherwise.
     *
     * @see destroy()
     * @see cleanUp()
     ...
     *
     * The method will remove any of the calling node's children that is marked
     * for removal (thus detaching them from its parent), and then do the same
