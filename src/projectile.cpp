...
const float poopSpeed = 500.0f, bananaSpeed = 600.0f;

Projectile::Projectile(const TextureHolder &textures, Type type)
    : type(type), counter(sf::Time::Zero) {
    switch (type) {
    case Poop:
...
Projectile::Type Projectile::getType() const { return type; }
...
Category::Type Projectile::getCategory() const { return Category::Projectile; }
