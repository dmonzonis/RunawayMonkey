...
StateManager::StateManager(State::Context context)
    : stateStack(), pendingChanges(), context(context), stateFactory() {}
...
void StateManager::handleEvent(const sf::Event &event) {
    ...
    for (auto it = stateStack.rbegin(); it != stateStack.rend(); ++it) {
        // If the state returns false on handleEvent, it doesn't want to let
        // lower
...
void StateManager::popState() { pendingChanges.push_back(Change(Pop)); }

void StateManager::clearStates() { pendingChanges.push_back(Change(Clear)); }

bool StateManager::isEmpty() const { return stateStack.empty() ? true : false; }
